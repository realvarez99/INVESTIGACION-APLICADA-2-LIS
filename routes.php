...
function validateBookData($data): ?string
{
    ...
    if (isset($data['year']) && (!is_numeric($data['year']) || (int) $data['year'] < 1000 || (int) $data['year'] > $currentYear)) {
        return "El campo 'year' debe ser un año válido entre 1000 y $currentYear";
    }

    
    if (isset($data['isbn'])) {
        
        $isbnClean = str_replace(['-', ' '], '', $data['isbn']);
        
        
        if (!preg_match('/^(?:\d{9}[\dX]|\d{13})$/i', $isbnClean)) {
            return "El formato del ISBN es inválido. Debe contener 10 o 13 dígitos válidos.";
        }
    }
